...
void object_recognizer::initialize_publishers()
{
    ...
    objectRT_publisher_= nh_.advertise<geometry_msgs::Pose>( "/objectRT", 1, true ); //&object_pose
}
...
void object_recognizer::kinectCB( const sensor_msgs::PointCloud2ConstPtr & cloud )
{
    ...
    //std::cout<<"got kinect cloud"<<std::endl;
...
void object_recognizer::timerCB( const ros::TimerEvent & )
{
    ...
    if (rototranslations.size() > 0)
    {
        ...
        //objectRT_publisher_.publish(object_pose);
    }
...
// void object_recognizer::cloud_cb (const pcl::PCLPointCloud2ConstPtr& cloud_msg){
...
//     this->set_scene_cloud(*cloud_msg);
//    if (this->find_best(rotation, translation))
//     {
...
//     }
//   //pub_object_pose.publish (*cloudPtrx);
//  }